...
function startRecursive() {
  ...
  let total = 0;
  const fetchedUsers = users.splice(0, 10);
  return Promise.all(fetchedUsers.map(user => processUser(user.id)))
    .then(resArr => {
      total += resArr.reduce((prev, cur) => prev + cur);
      return users.length !== 0 ?
        startRecursive().then(res => total += res) : total;
    });
}
